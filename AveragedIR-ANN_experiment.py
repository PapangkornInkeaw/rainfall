...
def prepareData4Station(lat,long,timestamp):
    data = np.empty((1,4))
    tmpData = np.empty((1,len(timestamp)))
    i=0
    n=0
    for t in timestamp:
        year = t.year
        ...
        if path.exists(ncfiledir_h8) or path.exists(ncfiledir_h9):
            ...
            tmpData[0,i] = satData.interp(latitude=lat, longitude=long).get('tbb').values
            n = n+1
        else:
            tmpData[0,i] = 0
        i = i+1
    data[0,0] = np.sum(tmpData)/n
    data[0,1] = lat
    data[0,2] = long
    dem = getDem(lat,long)
    data[0,3] = dem
...
def runExpriment(year, month, day):
    ...
    for idx in selectIdx:
        ...
        dsx[i,:] = prepareData4Station(round(lat,6),round(long,6),timestamp)
